...
export = () => {
	it("should kill snake on collision", () => {
		store.addSnake("__test1__", { position: new Vector2(0, 0) });
		store.addSnake("__test2__", { position: new Vector2(0, 100) });
		onSnakeTick();
		store.patchSnake("__test1__", { position: new Vector2(0, 100) });
		onCollisionTick();
	...
	it("should not kill snake on self-collision", () => {
		store.addSnake("__test__", { position: new Vector2(0, 0) });
		onSnakeTick();
		onCollisionTick();
	...
	it("should not kill snake when collided with dead snake", () => {
		store.addSnake("__test1__", { position: new Vector2(0.1, 100) });
		store.addSnake("__test2__", { position: new Vector2(0, 100) });
		onSnakeTick();
	...
	it("should kill snakes out of bounds", () => {
		store.addSnake("__test__", { position: new Vector2(0, WORLD_BOUNDS + 1) });
