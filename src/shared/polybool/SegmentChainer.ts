...
export function joinLines(seg1: SegmentLine, seg2: SegmentLine, geo: Geometry): SegmentLine | false {
...
export function joinCurves(seg1: SegmentCurve, seg2: SegmentCurve, geo: Geometry): SegmentCurve | false {
	if (geo.isCollinear(seg1.p2, seg1.p3, seg2.p1)) {
		...
		if (ts !== 0 && ts !== 1) {
			const ns = new SegmentCurve(
				seg1.p0,
				[seg1.p0[0] + (seg1.p1[0] - seg1.p0[0]) / t, seg1.p0[1] + (seg1.p1[1] - seg1.p0[1]) / t],
				[
...
export function joinSegments(seg1: Segment | undefined, seg2: Segment | undefined, geo: Geometry): Segment | false {
	if (seg1 === seg2) {
		return false;
	}
