...
function StoryContent({ current, target }: { current: number; target: number }) {
	const rem = useRem();
	...
	return (
		<frame BackgroundTransparency={1} Size={new UDim2(0, rem(22), 0, rem(6))} Position={new UDim2(0.5, rem(-11), 0.5, rem(-3))}>
			<MilestoneItem data={data} celebrating={false} />
		</frame>
	);
}
...
const story = {
	...
	story: (props: InferFusionProps<typeof controls>) => {
		...
		return <StoryContent current={c.current as number} target={c.target as number} />;
	},
};
